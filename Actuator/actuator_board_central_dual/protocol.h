...
int receive_message(message_t *msg) {
  ...
  do {
    ...
    ok = clientUart.readBytes(&msg->protocol_header[2], 1);
  } while (not ok || msg->protocol_header[2] != PROTOCOL_HDR2);

  // read length
  ok = clientUart.readBytes(&msg->length, 1);
  if (not ok)
    return 0;
  ...
  // read data
  return clientUart.readBytes(reinterpret_cast<unsigned char *>(msg->data),
