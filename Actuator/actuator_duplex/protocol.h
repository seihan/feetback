...
int receive_message(message_t *msg) {
  ...
  do {
    while (not ok || msg->protocol_header[0] != PROTOCOL_HDR1)
      ok = clientUart.readBytes(&msg->protocol_header[0], 1);
    ok = clientUart.readBytes(&msg->protocol_header[1], 1);
  } while (not ok || msg->protocol_header[1] != PROTOCOL_HDR2);

  // read length
  ok = clientUart.readBytes(reinterpret_cast<unsigned char *>(&msg->length), sizeof(msg->length));
  if (not ok)
    return 0;
  ...
  // read data
  return clientUart.readBytes(reinterpret_cast<unsigned char *>(msg->data), sizeof(msg->data));
}
