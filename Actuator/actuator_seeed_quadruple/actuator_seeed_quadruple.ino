...
/**
   ...
   arrived.
*/
void bleuart_rx_callback(BLEClientUart& bleuart)
{
  // uart_svc is prphs[conn_handle].bleuart
  uint16_t conn_handle = bleuart.connHandle();
  ...
  {
    //int bytesread = 0;
    sender[ 0 ] = peer->name[ 0 ];
    if ( bleuart.read(buf, sizeof(buf) - 1) ) {} // Serial.println(buf);
  }
}
...
void loop()
{
  ...
  {
    int values[ 2 ] = {0, 0};
    values[ 0 ] = atoi(strtok(buf, ";:,"));
    values[ 1 ] = atoi(strtok(NULL, ";:,"));
    switch ( sender[ 0 ] ) {
      case 'L' :
        if ( (values[ 0 ] > 0) && (values[ 1 ] > 0) ) {
          Serial.println("From Left: " + String(values[ 0 ]) + "\t" + String(values[ 1 ]));
        }
        if ( (values[ 0 ] > 0) {
        analogWrite(outputpins[ 0 ], map(values[ 0 ], 0, 4095, 0, 255));
        }
        if ( (values[ 1 ] > 0) {
        analogWrite(outputpins[ 1 ], map(values[ 0 ], 0, 4095, 0, 255));
        }
        break;
      case 'R' :
          if ( (values[ 0 ] > 0) && (values[ 1 ] > 0) ) {
          Serial.println("From Right: " + String(values[ 0 ]) + "\t" + String(values[ 1 ]));
          }
        if ( (values[ 0 ] > 0) {
        analogWrite(outputpins[ 2 ], map(values[ 0 ], 0, 4095, 0, 255));
        }
        if ( (values[ 1 ] > 0) {
        analogWrite(outputpins[ 3 ], map(values[ 0 ], 0, 4095, 0, 255));
        }
        break;
      default :
          break;
