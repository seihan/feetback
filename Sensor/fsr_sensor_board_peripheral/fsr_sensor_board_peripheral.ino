...
const long second = 1000;           // interval at which to blink (milliseconds)
int counter = 0;
...
void loop()
{
  ...
  //uint16_t values[ MAX_SERIAL ] = {};
  //msg.length = read_sole(values);
  msgsl.length = read_sole(msgsl.data);
  //  for (int i = 0; i < MAX_SERIAL; i++) msg.data[ i ] = map_n_cast( values[ i ] );
  send_to_serial(&msgsl);
  /*uint16_t values[ 956 ] = {};

    for (uint8_t& val : msg.data) val = 0;

    read_sole(values);

    for (int i = 0; i < 956; i++) {
    msg.data[ i ]  = map( constrain( values[ i ], 0, 4095 ), 100, 4095, 0, 255 );
